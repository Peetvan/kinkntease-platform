        ...
        case 'auth':
            switch ($action) {
                case 'register':
                    ...
                    // Create user
                    $stmt = $db->prepare("INSERT INTO users (username, email, password_hash, date_of_birth, gender, coins, created_at) VALUES (?, ?, ?, ?, ?, 100, NOW())");
                    $stmt->execute([$username, $email, hashPassword($password), $dob ?: null, $gender ?: null]);
                    ...
                    jsonResponse(true, ['user_id' => $userId, 'message' => 'Registration successful']);
                    break;
